...
def calculate_power(wind_speed_inner, wind_speed_wake, distance, rotor_radius=25):
    speed = wind_speed_inner
    speed *= (1 - ((1 - ((0.3 * wind_speed_wake) / wind_speed_inner)) * (
...
def calculate_power_cell(cone_grid, power_grid, WT_coordinate):
    ...
    WT_x = int(WT_x - 0.5)
    WT_y = int(WT_y - 0.5)
    if cone_grid[WT_x][WT_y] is None:
        ...
        if power_grid[WT_wake_x][WT_wake_y] is None:
            calculate_power_cell(cone_grid, power_grid, cone_grid[WT_x][WT_y][0])
        power_grid[WT_x][WT_y] = calculate_power(wind_speed_inner=wind_speed,
...
def calculate_power_grid(cone_grid, WT_coordinates):
    ...
    for WT in WT_coordinates:
        if power_grid[int(WT[0] - 0.5)][int(WT[1] - 0.5)] is not None:
            continue
